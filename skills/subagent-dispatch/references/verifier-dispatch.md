...
- Fresh-context verifier subagents tend to outperform self-critique (Anthropic Fable 5 scaffolding guidance, re-verified against the live page 2026-07-01). For long builds, the official pattern is interval verification: "Establish a method for checking your own work at an interval of [X] as you build. Run this every [X interval], verifying your work with subagents against the specification."
- Wiring a reviewer agent after the producer lifted SWE-bench resolution from 10.63% to 13.94% — +31% relative (MAGIS 2403.17927): the verifier gets spec and artifact, emits pass/fail with feedback, and the generator loops on fail with a capped iteration count.
- Keep the critic role pure: a dedicated reflector that only critiques added +4.3pp (55.1→59.4 on AppWorld, ACE 2510.04618). A verifier that also executes the next step or rewrites shared context stops being an independent check on either.
...
- **Default to the smallest verifiable loop before scaling the action space.** A run → observe-error → revise self-debug loop resolved 32.4% of ScienceAgentBench tasks at $0.057 each, beating a full browser+shell agent at 21.6% and $0.958 — ~17× cheaper for higher accuracy (2410.05080). Add tools to the verify loop only when the simple one is demonstrably insufficient; the advantage is model-specific (some models do better with the richer agent), so measure both and report cost alongside accuracy.
- **Check before acting on irreversible steps.** For a plan containing irreversible actions, a lightweight precondition check — enumerate the preconditions for the planned action, proceed only if all hold — prevents costly recovery (ProAgent 2308.11339; no isolated ablation, so treat it as a cheap guard rather than a proven lift).
- **For agents built on models you did not train, audit the tool-call trace, not just the output.** A backdoored agent can redirect intermediate tool calls while keeping the final output correct (Thought-Attack, 2402.11208): include the full tool-call sequence in the verifier dispatch and FAIL on unexpected tool destinations even when the output looks right. (Supply-chain context: skill-craft's review reference.)
...
5. **The verifier may be a weaker model.** Checking is easier than producing (prover-verifier games, 2407.13692). Downgrade one tier: opus producer → sonnet verifier, sonnet producer → haiku verifier for mechanical checks. Model-fit caveat for Opus 5 as the *reviewer*: one vendor benchmark measured lower issue coverage than its baseline (55.2% vs 61.1%) with ~4× the nitpicks, weakest on logic errors, race conditions, and API misuse (CodeRabbit, 2026-07-24 — single suite, launch-day). Keep rule 1's deterministic checks doing the correctness work on concurrency-heavy diffs, and don't let a single Opus 5 pass stand alone there.
...
7. **Cap verify → revise at two rounds.** If the second revision still fails, escalate to the user with both verdicts rather than looping. On the final retry, consider pairing the failure feedback with a known-good exemplar of the task type — a design hypothesis from SDPO (2601.20802: measured gains come from per-question *training*; inference-time transfer is unverified, so A/B it before adopting).
